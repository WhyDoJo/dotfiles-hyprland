...
function parseCssTexts(cssTexts) {
    ...
    return Promise.resolve(cssTexts).then(cssTexts => cssTexts.reduce((acc, cssText) => {
        try {
            acc.styleSheets.push((0, css_1.parse)(cssText));
        }
        catch (error) {
...
function getCSSSelectors(rules) {
    return Promise.resolve(rules).then(rules => {
        if (rules.length > 0) {
            return (0, arrayUtils_1.flatten)(rules.map(rule => rule.selectors)).filter(value => value && value.length > 0);
        }
        else {
            return [];
        }
...
function findMediaRules(cssAST) {
    ...
    if (mediaNodes.length > 0) {
        return (0, arrayUtils_1.flatten)(mediaNodes.map(node => node.rules));
    }
    else {
        return [];
    }
